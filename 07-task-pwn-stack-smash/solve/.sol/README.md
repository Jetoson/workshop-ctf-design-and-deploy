...
1.  **Build the Solver Image**
    ...
    ```console
    docker build -t stack-smash-solver .
    ```

1.  Ensure you have the binary in `../publish/stack_smash` (for local solving).
...
We mount the current directory (for `exploit.py`) and the `../publish` directory (for `stack_smash`).
...
It connects to `127.0.0.1:31600`.
...
docker run --rm -it --network host \
    ...
    -v "$(pwd)/../publish:/publish" \
    stack-smash-solver \
    python3 exploit.py REMOTE HOST=127.0.0.1 PORT=31600
```
...
```console
# Replace 1.2.3.4 and 31600 with actual target
docker run --rm -it \
    -v "$(pwd):/solve" \
    -v "$(pwd)/../publish:/publish" \
    stack-smash-solver \
    python3 exploit.py REMOTE HOST=1.2.3.4 PORT=31600
```
